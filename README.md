...
Можете да управлявате съдържанието на KV директно през `npx wrangler`:

```bash
npx wrangler kv:key put <ключ> "<стойност>" --binding=RESOURCES_KV
npx wrangler kv:key get <ключ> --binding=RESOURCES_KV
npx wrangler kv:key delete <ключ> --binding=RESOURCES_KV
```
