...
### Конфигуриране на AI модели и токени

Администраторският панел включва секция **AI конфигурация**,
която позволява бързо обновяване на използваните модели и токени.

1. Влезте в `admin.html` с администраторски акаунт.
2. Придвижете се до секцията „AI конфигурация“.
3. Попълнете полетата за `Token` и `Model` за генериране на план,
...
Токените за отделните AI модели могат да се променят от интерфейса,
защото се записват в `RESOURCES_KV`. Самият `WORKER_ADMIN_TOKEN`
обаче не може да се редактира през панела – необходимо е да го
зададете като *worker secret* преди деплой.

> **Важно**: Съхраняването на токени в KV улеснява конфигурацията,
> но не е препоръчително за чувствителни ключове.
> За по-висока сигурност използвайте Worker secrets.
