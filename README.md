...
Цветът на сиянието на прогрес баровете се управлява от CSS променливата `--progress-bar-glow-color`. При нужда задайте нова стойност:

```css
:root {
  --progress-bar-glow-color: rgba(255, 140, 0, 0.7); /* оранжев отблясък */
}
```
...
Процесът за актуализиране на хранителния план е опростен и не използва чат:

1. Натиснете бутона **Заяви промяна в плана** в седмичната секция на `code.html`. Ще се отвори формуляр *Промяна на плана* с кратки насоки.
2. Въведете свободен текст с желаните корекции (напр. повече протеин на обяд, изключване на алерген) и изпратете. Заявките, които не противоречат на здравословните принципи и BMI, се прилагат автоматично чрез AI директно върху `final_plan` без пълна регенерация. При конфликт се показва ясно съобщение.
