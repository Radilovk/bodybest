...
const loadComponent = () => {
  const html = readFileSync(new URL('../../macroAnalyticsCardStandalone.html', import.meta.url), 'utf8');
  const scriptContent = /<script type="module">([\s\S]*?)<\/script>/.exec(html)[1];
  new Script(scriptContent).runInThisContext();
};

describe('MacroAnalyticsCard component', () => {
  beforeEach(() => {
    if (!originalFetch) originalFetch = global.fetch;
    if (!originalIO) originalIO = global.IntersectionObserver;
    if (!customElements.get('macro-analytics-card')) loadComponent();
