...
describe('progress bar width handling', () => {
  ...
  test.each([
    ...
    await setup(val);
    const maskWidth = expectedWidth ? `${100 - parseInt(expectedWidth)}%` : '';
    expect(document.getElementById('goalProgressMask').style.width).toBe(maskWidth);
    expect(document.getElementById('engagementProgressMask').style.width).toBe(maskWidth);
    expect(document.getElementById('healthProgressMask').style.width).toBe(maskWidth);
