...
describe('processPendingUserEvents', () => {
  ...
  test('processes irisDiag event', async () => {
    const env = {
      USER_METADATA_KV: {
        list: jest.fn().mockResolvedValue({ keys: [{ name: 'event_irisDiag_u2' }] }),
        get: jest.fn().mockResolvedValue(JSON.stringify({ type: 'irisDiag', userId: 'u2', payload: { iris: 'imgdata' } })),
        delete: jest.fn(),
        put: jest.fn()
      }
    };
    ...
    expect(ctx.waitUntil).toHaveBeenCalledTimes(1);
    expect(env.USER_METADATA_KV.put).toHaveBeenCalled();
    expect(env.USER_METADATA_KV.delete).toHaveBeenCalledWith('event_irisDiag_u2');
  });
});
